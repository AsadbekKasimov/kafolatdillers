// ============================================
// НАСТРОЙКИ ДОСТУПА - ИЗМЕНИТЕ ЭТИ ПАРАМЕТРЫ
// ============================================

// URL вашей Google Sheets таблицы в формате JSON
// Инструкция по получению:
// 1. Откройте вашу Google таблицу
// 2. Файл -> Поделиться -> Опубликовать в интернете
// 3. Выберите формат "Веб-страница"
// 4. Замените YOUR_SHEET_ID на ID из URL вашей таблицы
const GOOGLE_SHEET_URL = 'https://docs.google.com/spreadsheets/d/1lBPp8N7j0faSoWjcWbKjQLnMwFS6YBIWZVJ94ZbhlYI/gviz/tq?tqx=out:json&sheet=Sheet1';

// Включить/выключить проверку доступа
...
class AccessControl {
    ...
    async loadAllowedUsers() {
        ...
        if (this.cache && (now - this.lastUpdate) < this.cacheTime) {
            console.log('Using cached allowed users');
            return this.cache;
        }

        try {
            console.log('Fetching allowed users from Google Sheets...');
            ...
            // Google Sheets возвращает JSONP, нужно извлечь JSON
            ...
            if (json.table && json.table.rows) {
                json.table.rows.forEach(row => {
                    if (row.c && row.c.length >= 4) {
                        const chatId = row.c[0]?.v; // Первая колонка - chat_id
                        const isActive = row.c[3]?.v; // Четвертая колонка - is_active
                        
                        if (chatId && isActive === true) {
                            allowedUsers.add(Number(chatId));
                        }
                    }
                });
            }

            console.log(`Loaded ${allowedUsers.size} allowed users`);
            this.cache = allowedUsers;
            this.lastUpdate = now;
            return allowedUsers;
    ...
    async isAllowed(userId) {
        try {
            const allowedUsers = await this.loadAllowedUsers();
...
const accessControl = new AccessControl(GOOGLE_SHEET_URL);
...
async function checkUserAccess() {
    // Если проверка доступа отключена, сразу показываем приложение
    ...
    try {
        ...
        // Проверяем доступ
        const hasAccess = await accessControl.isAllowed(userId);
...
function showAccessError(errorMessage) {
    document.getElementById('access-loading').style.display = 'none';
    document.getElementById('access-error').style.display = 'block';
    document.getElementById('error-message').textContent = errorMessage || 'Произошла ошибка при проверке доступа';
}
